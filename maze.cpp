...
bool Maze::bfs(bool deadBloc)
{
    ...
    while(!win)
    {
        ...
        regenererPos(temp.getPosPlayer(), temp.getField(), temp.getPosBoxes());//on remet le plateau à l'etat de temp
...
std::vector <bool> Maze::createZone()
{
    ...
    while(!maFile.empty())
    {
        ...
        for(int dir = 0; dir < 4; dir++)
        {
            ...
            if(!doublon(comb))//savoir si la situation s'est déja produite
            {
               ...
               // std::cout << " coucou i: " << i << " sauv " << sauvegarde.size() << " sauvT " << sauvegardeDeplacementJoueur.size() << " caseM " << caseMove(temp.m_pos_boxes,m_pos_boxes) << std::endl;
                if(!win)
                {
                    ...
                    if(!win)
                    {
                        ...
                        if(!doublon(sauvegardeDeplacementJoueur))//savoir si la situation s'est déja produite
                        {
